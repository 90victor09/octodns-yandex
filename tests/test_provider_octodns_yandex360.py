...
from octodns.record import PtrRecord, Create, Delete, Update
from octodns.zone import Zone

from octodns_yandex import Yandex360Provider
from octodns_yandex.yandex360_provider import map_record_to_entries, map_entries_to_records, Yandex360Exception, \
    Yandex360ApiException
from tests.fixtures.ya360 import STUB_ENTRIES, STUB_ORG_1, STUB_ORG_2, STUB_DOMAIN_1, STUB_DOMAIN_2, make_cursor_resp, \
    make_paginated_resp
...
class TestYandex360Provider:
    def test_make_request(self, provider, monkeypatch):
        ...
        with pytest.raises(Yandex360ApiException, match=f".*{MockSession.text}.*"):
    ...
    def test_api_urls(self, provider, monkeypatch):
        ...
        assert (last_method, last_url) == ('GET', "/directory/v1/org/123/domains")

        provider.list_dns_records(123, 'example.com')
        assert (last_method, last_url) == ('GET', '/directory/v1/org/123/domains/example.com/dns')

        provider.create_dns_record(123, 'example.com', {})
        assert (last_method, last_url) == ('POST', '/directory/v1/org/123/domains/example.com/dns')

        provider.update_dns_record(123, 'example.com', 12345, {})
        assert (last_method, last_url) == ('POST', '/directory/v1/org/123/domains/example.com/dns/12345')

        provider.delete_dns_record(123, 'example.com', 12345)
        assert (last_method, last_url) == ('DELETE', '/directory/v1/org/123/domains/example.com/dns/12345')

    def test_find_org_not_found(self, provider, monkeypatch):
        monkeypatch.setattr(provider, 'list_orgs', lambda page_token: make_cursor_resp('', organizations=[]))
    ...
    def test_find_org_multiple_pages(self, provider, monkeypatch):
        def _list_orgs(page_token):
            if not page_token:
                ...
                orgs = [STUB_ORG_2]
            return make_cursor_resp('2' if not page_token else '', organizations=orgs)
    ...
    def test_mapping_unknown(self):
        ...
        with pytest.raises(Yandex360Exception):
            map_entries_to_records(None, zone, False, [
                {
                    'recordId': 8,
        ...
        with pytest.raises(Yandex360Exception):
            map_record_to_entries(zone, PtrRecord(zone, "test", {
                'ttl': 600,
                'value': "example.com."
            }))
    ...
    def test_collect_zone_entries(self, provider, monkeypatch):
        ...
        entries = provider.collect_zone_entries(STUB_ORG_1['id'], STUB_DOMAIN_1['name'])
        assert entries == STUB_ENTRIES['A'] + STUB_ENTRIES['MX']

    def test_populate_not_found(self, provider, monkeypatch):
        zone = Zone('example.com.', [])

        monkeypatch.setattr(provider, 'find_org_id_for_domain', lambda domain_name: None)

        result = provider.populate(zone)
        assert not result

    def test_populate_found(self, provider, monkeypatch):
        TEST_ENTRIES = list(functools.reduce(lambda acc, x: acc + x, STUB_ENTRIES.values(), []))
        zone = Zone('example.com.', [])

        monkeypatch.setattr(provider, 'find_org_id_for_domain', lambda domain_name: STUB_ORG_1['id'])
        monkeypatch.setattr(
            provider,
    ...
    def _make_plan(create, delete, update):
        ...
        return Plan(
            ...
            changes=[
                ...
                *map(lambda x: Update(*x), map(lambda x: (
    ...
    def test_apply(self, provider, monkeypatch):
        TEST_ENTRIES = functools.reduce(lambda acc, x: acc + x, STUB_ENTRIES.values(), [])
        created_tuples, deleted_ids, updated_ids = [], [], []

        monkeypatch.setattr(provider, 'find_org_id_for_domain', lambda domain_name: STUB_ORG_1['id'])
        monkeypatch.setattr(provider, 'collect_zone_entries', lambda org_id, domain_name: TEST_ENTRIES)

        monkeypatch.setattr(provider, 'delete_dns_record',
                            lambda org_id, domain, record_id: deleted_ids.append(record_id))
        monkeypatch.setattr(provider, 'create_dns_record',
                            lambda org_id, domain, data: created_tuples.append((data['type'], data['name'])))
        monkeypatch.setattr(provider, 'update_dns_record',
                            lambda org_id, domain, record_id, data: updated_ids.append(record_id))

        provider.apply(self._make_plan(
            create=STUB_ENTRIES['A'] + STUB_ENTRIES['CNAME'],
            delete=STUB_ENTRIES['MX'] + STUB_ENTRIES['SRV'],
            update=[
                (STUB_ENTRIES['CNAME'], STUB_ENTRIES['CNAME_mail']),  # Basic update
                (STUB_ENTRIES['A'], STUB_ENTRIES['A_single']),  # Remove one entry
                (STUB_ENTRIES['AAAA'], STUB_ENTRIES['AAAA_triple']),  # Add one entry
        ...
        assert created_tuples == list(map(
            lambda x: (x['type'], x['name']),
            STUB_ENTRIES['A'] + STUB_ENTRIES['CNAME'] + STUB_ENTRIES['CAA'] + [STUB_ENTRIES['AAAA_triple'][2]]
        ))
        assert deleted_ids == list(map(
            lambda x: x['recordId'],
            STUB_ENTRIES['MX'] + STUB_ENTRIES['SRV'] + STUB_ENTRIES['CAA'] + [STUB_ENTRIES['A'][1]]
        ))
        assert updated_ids == list(map(
            lambda x: x['recordId'],
            [STUB_ENTRIES['A'][0]] + STUB_ENTRIES['AAAA'] + STUB_ENTRIES['CNAME']
        ))
