...
from yandex.cloud.dns.v1.dns_zone_service_pb2 import (
    ListDnsZonesResponse, ListDnsZoneRecordSetsResponse
)
...
from octodns_yandex.yandexcloud_provider import map_octodns_to_rset, \
    YandexCloudException, YandexCloudConfigException, \
    AUTH_TYPE_OAUTH, AUTH_TYPE_IAM, AUTH_TYPE_METADATA, AUTH_TYPE_SA_KEY, \
    AUTH_TYPE_YC_CLI, map_rset_to_octodns
from tests.fixtures.dns_zones import STUB_ZONE_NAME, STUB_ZONE_1, STUB_ZONE_2, STUB_ZONE_PUBLIC, \
    STUB_IDNA_ZONE, STUB_IDNA_ZONE_NAME, STUB_FOLDER_ID
...
def provider(disable_sdk):
    return YandexCloudProvider("test", folder_id=STUB_FOLDER_ID, auth_type=AUTH_TYPE_METADATA)


@pytest.fixture()
def provider_with_zone(monkeypatch, provider):
    def _list(request):
        return ListDnsZonesResponse(next_page_token='', dns_zones=[STUB_ZONE_PUBLIC])
...
class TestYandexCloudProvider:
    ...
    class TestAuth:
        ...
        def _get_auth_kwargs(auth_type, **kwargs):
            return YandexCloudProvider.get_auth_kwargs(
                auth_type,
                **{
                    'oauth_token': None,
                    'iam_token': None,
        ...
        def test_auth_oauth(self):
            auth_kwargs = self._get_auth_kwargs(AUTH_TYPE_OAUTH, oauth_token=STUB_TOKEN)
        ...
        def test_auth_service_account_key(self):
            ...
            with tempfile.NamedTemporaryFile('w+') as fp:
                ...
                fp.flush()
                auth_kwargs = self._get_auth_kwargs(AUTH_TYPE_SA_KEY, sa_key_file=fp.name)
                assert auth_kwargs.keys() == {'service_account_key'}
                assert auth_kwargs['service_account_key'] == STUB_SA_KEY

            with pytest.raises(YandexCloudConfigException, match=r".*non_existent_file.*"):
                self._get_auth_kwargs(AUTH_TYPE_SA_KEY, sa_key_file='non_existent_file')

            # Auth using Service Account key, provided in config
            auth_kwargs = self._get_auth_kwargs(AUTH_TYPE_SA_KEY, sa_key=STUB_SA_KEY)
            assert auth_kwargs.keys() == {'service_account_key'}
            assert auth_kwargs['service_account_key'] == STUB_SA_KEY
    ...
    class TestFindZone:
        def test_find_zone_found_one(self, monkeypatch, provider):
            def _list(request):
                return ListDnsZonesResponse(next_page_token='', dns_zones=[STUB_IDNA_ZONE])
        ...
        def test_find_zone_mapped(self, monkeypatch, disable_sdk):
            def _list(request):
                return ListDnsZonesResponse(next_page_token='', dns_zones=[STUB_ZONE_1, STUB_ZONE_2, STUB_ZONE_PUBLIC])
            ...
            def _list_idna(request):
                return ListDnsZonesResponse(next_page_token='', dns_zones=[STUB_ZONE_1, STUB_ZONE_2, STUB_IDNA_ZONE])
        ...
        def test_find_zone_multiple_public(self, monkeypatch, disable_sdk):
            def _list(request):
                return ListDnsZonesResponse(next_page_token='', dns_zones=[STUB_ZONE_1, STUB_ZONE_PUBLIC, STUB_ZONE_2])
            provider = YandexCloudProvider("test", folder_id=STUB_FOLDER_ID, auth_type=AUTH_TYPE_METADATA,
            ...
            def _list_without_public(request):
                return ListDnsZonesResponse(next_page_token='', dns_zones=[STUB_ZONE_1, STUB_ZONE_2])
            monkeypatch.setattr(provider.dns_service, 'List', _list_without_public)
            zone_id = provider.get_zone_id_by_name(STUB_ZONE_NAME)
            assert zone_id == STUB_ZONE_1.id

        def test_find_zone_multiple_private(self, monkeypatch, disable_sdk):
            def _list(request):
                return ListDnsZonesResponse(next_page_token='', dns_zones=[STUB_ZONE_PUBLIC, STUB_ZONE_1, STUB_ZONE_2])
            provider = YandexCloudProvider("test", folder_id=STUB_FOLDER_ID, auth_type=AUTH_TYPE_METADATA,
    ...
    class TestMapping:
        ...
        def test_mapping_lenient(self):
            zone = Zone(STUB_ZONE_NAME, [])
            records = map(lambda x: STUB_RECORDS[x], (
                'CAA_quoted', 'TXT_quoted',
                'CNAME_nodot', 'CNAME_dot',
                'MX_nodot', 'SRV_nodot', 'ANAME_nodot',
            ))
            for rset in records:
        ...
        def test_mapping_unknown(self):
            with pytest.raises(YandexCloudException):
                map_rset_to_octodns(None, None, True, RecordSet(
    ...
    class TestPopulate:
        ...
        def test_populate_single_page(self, monkeypatch, provider_with_zone):
            def _list_recordsets(request):
                return ListDnsZoneRecordSetsResponse(next_page_token='', record_sets=[STUB_RECORDS['A'], STUB_RECORDS['SOA']])

            monkeypatch.setattr(provider_with_zone.dns_service, 'ListRecordSets', _list_recordsets)

            zone = Zone(STUB_ZONE_NAME, [])
            b = provider_with_zone.populate(zone)
            assert b
            assert len(zone.records) == 1  # SOA ignored because it is not on provider's (and octodns) SUPPORTS list
        ...
        def test_populate_multiple_pages(self, monkeypatch, provider_with_zone):
            ...
            def _list_recordsets(request):
                ...
                if next_offset >= len(TEST_RECORDS):
                    next_offset = ''
                return ListDnsZoneRecordSetsResponse(next_page_token=str(next_offset), record_sets=rsets)
    ...
    class TestApply:
        ...
        def _make_plan(create, delete, update):
            ...
            return Plan(
                ...
                desired=zone,
                changes=[
                    *map(lambda x: Create(map_rset_to_octodns(None, zone, True, x)), create),
                    *map(lambda x: Delete(map_rset_to_octodns(None, zone, True, x)), delete),
                    *map(lambda x: Update(
        ...
        def _compare_rset_batches(result, expected):
            ...
            expected_str = sorted(map(lambda x: x.SerializeToString(), expected))
        ...
        def test_apply_simple(self, monkeypatch, provider_with_zone):
            ...
            monkeypatch.setattr(provider_with_zone.dns_service, 'UpdateRecordSets', _update_record_sets)

            # Large UPDATE_CHUNK_SIZE will put update in the same operation as delete & create
            provider_with_zone.apply(self._make_plan(
                delete=[STUB_RECORDS['A']],
                create=[STUB_RECORDS['MX']],
                update=[(STUB_RECORDS['CNAME_nodot'],STUB_RECORDS['CNAME'])],
            ))
            assert len(additions) == 1
            assert len(deletions) == 1

            self._compare_rset_batches(deletions[0], [STUB_RECORDS['A'], STUB_RECORDS['CNAME_nodot']])
            self._compare_rset_batches(additions[0], [STUB_RECORDS['MX'], STUB_RECORDS['CNAME']])
        ...
        def test_apply_big(self, monkeypatch, provider_with_zone):
            ...
            batch3_delete = [STUB_RECORDS['PTR_nodot'], STUB_RECORDS['SRV_nodot']]
        ...
        def test_apply_error(self, monkeypatch, provider_with_zone):
            ...
            with pytest.raises(YandexCloudException):
                provider_with_zone.apply(self._make_plan([STUB_RECORDS['A']], [], []))
